    ...
    <div className="min-h-screen bg-[#FFEFEF] flex">
      ...
      <div className="w-16 border-r border-pink-100 h-screen flex flex-col items-center py-6 space-y-8">
        <button onClick={() => handleNavigation("/user")}>
          <User
            className={`w-6 h-6 ${
              activePage === "/user" ? "text-pink-400" : "text-gray-400"
            }`}
          />
        </button>
        <button onClick={() => handleNavigation("/user/calendar")}>
          <Calendar
            className={`w-6 h-6 ${
              activePage === "/user/calendar"
                ? "text-pink-400"
                : "text-gray-400"
            }`}
          />
        </button>
        <button onClick={() => handleNavigation("/user/history")}>
          <Clock
            className={`w-6 h-6 ${
              activePage === "/user/history" ? "text-pink-400" : "text-gray-400"
            }`}
          />
        </button>
      </div>
      ...
      <div className="flex-1 p-8">
        ...
        <div
          className={`bg-white shadow-sm rounded-xl transition-all duration-500 ${
            isHistoryExpanded ? "h-[400px]" : "h-[200px]"
          }`}
        >
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100">
                  <th className="py-4 text-left font-medium text-gray-600">
                    Date
                  </th>
                  <th className="py-4 text-left font-medium text-gray-600">
                    Facility
                  </th>
                  <th className="py-4 text-right font-medium text-gray-600">
                    Points
                  </th>
                </tr>
              </thead>
